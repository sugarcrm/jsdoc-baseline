...
PublishJob.prototype.generateTocData = function(options) {
    ...
    // If there are externals, force their TOC to be last.
    _.each(navTree, function(item, key) {
        (item.doclet && item.doclet.kind === 'external' ? externalsItems : navTreeItems)[key] = item;
    });

    addItems(navTreeItems);
    addItems(externalsItems);
