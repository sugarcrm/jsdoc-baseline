...
DocletHelper.prototype.addDoclets = function(taffyData) {
    ...
    // adding doclet reference to navigation tree items.
    docletHash = {};
    doclets.forEach(function(item) {
        docletHash[item.longname] = item;
    });

    this.navTree = helper.longnamesToTree(this.getOutputLongnames(), docletHash);
